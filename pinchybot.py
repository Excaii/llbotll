     ...
     if h.status_code != 200:
      ...
      if re.match("(image\S+?(?P<format>(jpeg)|(png)|(gif)))", content_type):
       ...
       if int(h.headers["content-length"]) > 4194304: #Ignore if bigger than 4 MiB
        ...
        f = requests.get(url, stream=False)
        s = Image.open(f.raw)
...
################################################################
      ...
      if cmd == 'whoami':
       ...
       room.message("The llbotll script is {size} bytes in size, and {lines} lines long".format(size=str(size.st_size),lines=lines))
       ...
       room.message("Komutlar Profilimde ^^")
       ...
       try:
        ...
        room.message("Örnek kullanım: $roll 2d6")
        print(traceback.format_exc())
...
################################################################
     if unescaped_message.startswith("anon"):
      dish = random.randint(1, 1)
      if dish == 1:
       room.message("Geldiğin gibi git lütfen @anon!")
      else:
       print("No.")

     elif unescaped_message.startswith("Sa"):
      wat = random.randint(1, 1)
      if wat == 1:
       room.message("as, hoşgeldin^^.")
      else:
       print("No.")

     elif unescaped_message.startswith("Sea"):
      wat = random.randint(1, 1)
      if wat == 1:
       room.message("as, hoşgeldin^^.")
      else:
     ...
     elif unescaped_message.startswith("Merhaba"):
      wat = random.randint(1, 1)
      if wat == 1:
       room.message("merhabaa!, hoşgeldin^^.")
      else:
       print("No.")

     elif unescaped_message.startswith("Bot"):
      wat = random.randint(1, 1)
      if wat == 1:
       room.message("adım geçti galiba^^.")
      else:
       print("No.")

     elif unescaped_message.startswith("Nasılsınız"):
      wat = random.randint(1, 1)
      if wat == 1:
       room.message("iyi olmaya çalışıyorum*-*,sen nasılsın^^.")
      else:
       print("No.")
     ...
     elif unescaped_message.startswith("xD"):
      wat = random.randint(1, 1)
      if wat == 1:
       room.message("(xD)")
      else:
...
################################################################
      ...
      if cmd == "hi":
       ...
       pm.message(user, " Merhaba ben python diliyle yazılmış chatango botu llbotll ! Komut listesi profilimde! ")
